...
import { BUSD, CAKE, USDC, MMO } from '@pancakeswap/tokens'
...
const cakeBusdPairMap = {
  ...
  [ChainId.MATIC]: {
    address: Pair.getAddress(MMO[ChainId.MATIC], USDC[ChainId.MATIC]),
    tokenA: USDC[ChainId.MATIC],
    tokenB: MMO[ChainId.MATIC],
  }
}

const getCakePrice = async (isTestnet: boolean) => {
  const pairConfig = cakeBusdPairMap[isTestnet ? ChainId.BSC_TESTNET : ChainId.MATIC]
