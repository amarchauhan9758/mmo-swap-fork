...
export const multicallAddresses = {
  ...
  137: '0x2FFd4c4ff5Ea683d68515e2FebCa0FB503E9F7b7',
}
...
export function createMulticall<TProvider>(provider: ({ chainId }: { chainId?: number | undefined }) => TProvider) {
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.MATIC) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.MATIC, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.MATIC, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
