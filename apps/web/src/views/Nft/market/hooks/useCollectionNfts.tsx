...
const fetchAllNfts = async (
  ...
  if (settings.field !== 'tokenId' && !fallbackMode) {
    const marketDataNfts = await fetchMarketDataNfts(collection, settings, Number(page), tokenIdsFromFilter)
  ...
  if (fallbackMode) {
    ...
    tokenIds = tokenIdsFromFilter.slice(Number(page) * REQUEST_SIZE, (Number(page) + 1) * REQUEST_SIZE)
  } else {
    collectionNftsResponse = await getNftsFromCollectionApi(collection.address, REQUEST_SIZE, Number(page) + 1)
...
export const useCollectionNfts = (collectionAddress: string) => {
  ...
  const {
    ...
    async (address, settingsJson, page) => {
      const settings: ItemListingSettings = JSON.parse(settingsJson.toLocaleString())
      ...
      if (settings.showOnlyNftsOnSale) {
        newNfts = await fetchMarketDataNfts(collection, settings, Number(page), tokenIdsFromFilter)
      } else {
        const {
          nfts: allNewNfts,
