...
const ContributeButton: React.FC<React.PropsWithChildren<Props>> = ({ poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  const handleContributeSuccess = async (amount: BigNumber, txHash: string) => {
    await Promise.all([walletIfoData.fetchIfoData()])
    toastSuccess(
      t('Success!'),
      <ToastDescriptionWithTx txHash={txHash}>
        {t('You have contributed %amount% %token% to this IWO!', {
          amount: getBalanceNumber(amount),
          token: ifo.currency.symbol
        })}
  ...
  return (
    ...
    >
      {isMaxCommitted && publicIfoData.status === 'live' ? t('Max. Committed') : t('Contribute %symbol%' , { symbol: currency.symbol })}
    </Button>
  )
}
