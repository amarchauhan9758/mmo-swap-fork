...
import { PublicIfoData, WalletIfoData } from 'views/Iwo/types'
      ...
      <Message variant="warning" mb="16px">
        <Box>
          <Text fontSize="14px" color="#D67E0A">
            {t('This IWO has token vesting. Purchased tokens are released over a period of time.')}
          </Text>
...
const ContributeModal: React.FC<React.PropsWithChildren<Props>> = ({
      ...
      onApproveSuccess: ({ receipt }) => {
        toastSuccess(
          ...
          <ToastDescriptionWithTx txHash={receipt.transactionHash}>
            {t('You can now participate in the %symbol% IWO.', { symbol: ifo.token.symbol })}
  ...
  const basicTooltipContent = t(
    'For the private sale, each eligible participant will be able to commit any amount of CAKE up to the maximum commit limit, which is published along with the IWO voting proposal.',
  )
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          ...
          {/* <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough CAKE, you may not receive a meaningful amount of IWO tokens, or you may not receive any IWO tokens at all.',
            )}
            <Link
              fontSize="12px"
              display="inline"
