...
export const fetchPoolsAllowance = async (account) => {
  const calls = nonBnbPools.map((pool) => ({
    ...
    name: 'allowance',
    params: [account, getAddress(pool.contractAddress, 137)],
  })) 

  const allowances = await multicall(erc20ABI, calls, 137)
  // console.log("callscalls", allowances);
...
export const fetchUserBalances = async (account) => {
  ...
  const [tokenBalancesRaw, bnbBalance] = await Promise.all([
    multicall(erc20ABI, calls, 137),
    maticRpcProvider.getBalance(account),
  ])
  ...
  // console.log("fetchUserBalances", poolTokenBalances);
...
export const fetchUserStakeBalances = async (account) => {
  const calls = nonMasterPools.map((p) => ({
    address: getAddress(p.contractAddress, 137),
    name: 'userInfo',
    params: [account],
  }))
  const userInfo = await multicall(sousChefABI, calls, 137)
  return fromPairs(
...
export const fetchUserPendingRewards = async (account) => {
  ...
  const res = await multicall(sousChefABI, calls, 137)
