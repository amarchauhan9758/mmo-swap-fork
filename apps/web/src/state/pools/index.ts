...
import { bscRpcProvider, maticRpcProvider } from 'utils/providers'
...
const cakeVaultAddress = getCakeVaultAddress(137)
console.log("calevault", cakeVaultAddress);
...
export const fetchCakePoolUserDataAsync = (account: string) => async (dispatch) => {
  ...
  try {
  const allowanceCall = {
    address: maticTokens.mmo.address,
    name: 'allowance',
  ...
  console.error('[Pools Action Cake Pool User Data] error when getting staking limits', error)
}
}
...
export const fetchPoolsPublicDataAsync = (currentBlockNumber: number, chainId: number) => async (dispatch, getState) => {
    ...
    try {
      ...
      const [blockLimits, totalStakings, profileRequirements, currentBlock] = await Promise.all([
        ...
        fetchPoolsProfileRequirement(), 
        currentBlockNumber ? Promise.resolve(currentBlockNumber) : maticRpcProvider.getBlockNumber(),
      ])  
      ...
      // console.log("priceHelperLpsConfig",priceHelperLpsConfig);
      ...
      console.log("activePriceHelperLpsConfig", activePriceHelperLpsConfig);
      

      const poolsWithDifferentFarmToken =
        activePriceHelperLpsConfig.length > 0 ? await fetchFarms(priceHelperLpsConfig, chainId) : []
      
      console.log("poolsWithDifferentFarmToken", poolsWithDifferentFarmToken);
      ...
      const bnbBusdFarm =
        activePriceHelperLpsConfig.length > 0
          ? farmsData.find((farm) => farm.token.symbol === 'USDC' && farm.quoteToken.symbol === 'WMATIC')
          : null

        console.log("bnbBusdFarm", bnbBusdFarm);  

      const farmsWithPricesOfDifferentTokenPools = bnbBusdFarm
        ? getFarmsPrices([bnbBusdFarm, ...poolsWithDifferentFarmToken], chainId)
        : []

        console.log("farmsWithPricesOfDifferentTokenPools", farmsWithPricesOfDifferentTokenPools);
      ...
      console.error('[Pools Actions] error when getting public data', error)
    }
  }
...
>('pool/fetchPoolsUserData', async (account, { rejectWithValue }) => {
  try {
    ...
    console.log("fetchPoolsUserDataAsync1", pendingRewards); 
...
export const fetchCakeVaultFees = createAsyncThunk<SerializedVaultFees>('cakeVault/fetchFees', async () => {
  const vaultFees = await fetchVaultFees(getCakeVaultAddress(137))
  return vaultFees
})
...
export const fetchCakeFlexibleSideVaultFees = createAsyncThunk<SerializedVaultFees>(
  ...
  async () => {
    const vaultFees = await fetchVaultFees(getCakeFlexibleSideVaultAddress(137))
    return vaultFees
  },
)
